import { Certs } from '../../interfaces/skillsInterfaces'

export const certs: Certs[] = [
  {
    name: 'Udemy React 16 Course',
    img: 'udemyBurgerBuilder.webp',
    link: 'https://www.udemy.com/certificate/UC-A5AOU8C4/',
  },
  {
    name: 'AWS Cloud Practitioner',
    img: 'AWSCPBadge.webp',
    link: 'https://www.credly.com/badges/aeaec428-0a39-4245-8bb6-67851fa02904/public_url ',
  },
  {
    name: 'Free Code Camp JS course',
    img: 'FreeCodeCampCert.webp',
    link: 'https://www.freecodecamp.org/certification/fcc99c3375c-a5d4-4e8a-ba4a-96473ca87aa8/javascript-algorithms-and-data-structures',
  },
  {
    name: 'Free Code Camp C# Foundational',
    img: 'FreeCodeCampCSharp.webp',
    link: 'https://www.freecodecamp.org/certification/fcc99c3375c-a5d4-4e8a-ba4a-96473ca87aa8/foundational-c-sharp-with-microsoft',
  },
]
